...
def plot_results(x, y, params, classifier, title="", img_save_path=None, show_img=True):
    ...
    for ind in range(x.shape[0]):
        ...
        plt.plot(x_low_dim[ind, 0], x_low_dim[ind, 1], mshape, c=color, markerfacecolor='none', markeredgewidth=0.4,
                 markersize=4)
...
def smo(kernel_matrix, y, lamb, b, max_iter=10000, C=np.Infinity):
    ...
    while True:
        ...
        ba = b - Ea + (lamba_old - lamba_new) * y[inda] * kernel_matrix[inda][inda] + (lambb_old - lambb_new) * y[
            indb] * kernel_matrix[indb][inda]
        bb = b - Eb + (lamba_old - lamba_new) * y[inda] * kernel_matrix[inda][indb] + (lambb_old - lambb_new) * y[
            indb] * kernel_matrix[indb][indb]
...
def train_svm(x, y, C=np.Infinity, kernel_type=None, max_epoch=100, smo_iter_per_epoch=10000, epsilon=1e-7,
    ...
    for epoch in range(max_epoch):
        ...
        if plot_training_results:
            ...
            def my_classifier(input_x, decision_mode="hard"):
                return svm_classifier(x_support, y_support, lamb_support, b, input_x, kernel_type=kernel_type,
                                      decision_mode=decision_mode)

            plot_results(x, y, params, my_classifier, title="epoch %d" % (epoch),
                         img_save_path=generate_folder("results/smo-svm/") + "results-epoch%d.jpg" % (epoch),
                         show_img=False)
...
params, my_classifier = train_svm(x, y, C=10, plot_training_results=True,
                                  kernel_type={"name": "GAUSSIAN", "params": [within_class_average_distance(x, y)]})
